...
- (instancetype)initWithTitle:(NSString *)title
    ...
    NSArray *items =@[
                      MMItemMake(cancelTitle, MMItemTypeNormal, nil),
                      MMItemMake(otherTitle, MMItemTypeHighlight, nil)
                      ];
                 ...
                 inputHandler:(MMPopupInputHandler)inputHandler
{
    ...
    if ( self )
    {
        ...
        [tempItems enumerateObjectsUsingBlock:^(MMPopupItem * obj, NSUInteger idx, BOOL *stop){
            if(obj.title == nil){
                [tempItems removeObjectAtIndex:idx];
                *stop = YES;
            }
        }];
